...
def muzero_loss(
    ...
    policy_targets: Float[Array, "horizon num_actions"] = jax.nn.softmax(
        trajectory.logits, axis=-1
    )
    action_off, reward_off, done_off, policy_targets_off = (
        roll(trajectory.action),
        roll(trajectory.reward),
        roll(trajectory.obs_with_done.done),
        roll(policy_targets),
    )

    # e.g. values[i, j] is the predicted value at time i+j,
    # based on the observation at time i
    rewards, logits, values = jax.vmap(networks)(
        trajectory.obs_with_done.obs, action_off
    )

    # discard final transition
    # (too much hassle to pass around final transition)
    target_values: Float[Array, "length length-1"] = jax.vmap(
        rlax.n_step_bootstrapped_returns, (0, 0, 0, None)
    )(
        reward_off[:, :-1],
        jnp.where(done_off[:, 1:], 0.0, config.discount),
        values[:, 1:],
        config.num_steps_bootstrap,
    )

    # top left of matrix
    mask = (horizon_axis[:, None] + horizon_axis[None, :]) < horizon_axis.size

    # value loss
    value_losses = optax.l2_loss(values[:-1, :-1], target_values[:-1, :])
    value_loss = jnp.mean(value_losses, where=mask[:-1, :-1])

    # policy loss
    policy_losses = jax.vmap(optax.softmax_cross_entropy)(logits, policy_targets_off)
    policy_loss = jnp.mean(policy_losses, where=mask)

    # reward model loss
    reward_losses = optax.l2_loss(rewards, reward_off)
            ...
            def epoch_step(param_state: ParamState, key: rand.PRNGKey):
                ...
                def gradient_step(
                    ...
                    def loss_fn(params: Networks):
                        ...
                        loss, aux = jax.vmap(muzero_loss, (None, 0, None))(
