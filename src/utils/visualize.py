...
def visualize_callback(
    ...
    if predicted_rewards is not None:
        rows_per_env += 1
    num_value_bins = value_cfg.num_value_bins
    if num_value_bins != "scalar":
        rows_per_env += 1
    ...
    for i in range(num_envs):
        ...
        if predicted_rewards is not None:
            ...
            plot_compare_dists(
                ...
                value_cfg.value_to_probs(trajectories.timestep.reward[i]),
                labels=bin_labels,
            )
        ...
        if num_value_bins != "scalar":
            ...
            plot_compare_dists(
                ...
                value_cfg.value_to_probs(bootstrapped_returns[i]),
                labels=bin_labels,
            )
    ...
    if video is not None:
        ...
        for i in range(num_envs):
            for h in range(horizon):
                ...
                a = get_action_name(env_name, trajectories.action[i, h].item())
                # c h w -> h w c
...
def plot_statistics(
    ...
    if value_cfg.num_value_bins != "scalar":
