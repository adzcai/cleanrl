...
class Network(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.value_net = eqx.nn.MLP(obs_size, num_value_bins, mlp_size, mlp_depth, jax.nn.tanh, key=value_key)
        self.policy_net = eqx.nn.MLP(obs_size, num_actions, mlp_size, mlp_depth, jax.nn.tanh, key=policy_key)
...
def make_train(config: Config):
    ...
    def train(key: jr.PRNGKey):
        ...
        # initial states
        obs, env_state = jax.vmap(env.reset, (0, None))(jr.split(key_reset, config.num_envs), env_params)
        ...
        def iterate(iter_state: IterState, key: jr.PRNGKey):
            ...
            rollout_states, (trajectories, advantages, value_targets) = jax.vmap(rollout, (None, 0, 0))(
            ...
            def optimize_step(
                ...
                updates, opt_state = optim.update(grads, params_state.opt_state, params_state.params)
    ...
    def rollout(net: Network, init_rollout_state: RolloutState, key: jr.PRNGKey):
        ...
        def rollout_step(rollout_state: RolloutState, key: jr.PRNGKey):
            ...
            obs, env_state, reward, done, info = env.step(key_step, rollout_state.env_state, action, env_params)

            return RolloutState(obs, env_state), Transition(rollout_state, action, reward, done, preds)
    ...
    def loss_trajectory(
        ...
        td_error = tx_pair.apply_inv(value_target_probs) - tx_pair.apply_inv(jax.nn.softmax(preds.value_logits))
