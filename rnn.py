...
from jaxtyping import Bool, Integer, Float, Array, PyTree
...
class Config:
    ...
    num_total_transitions: int = 10_000
    max_horizon: int = 120
...
def make_train(config: Config):
    ...
    env: Environment = LogWrapper(env)
    ...
    def train(key):
        key_reset, key_network, key_hidden = rand.split(key, 3)
        ...
        def iteration_step(update_state: UpdateState, key: rand.PRNGKey):
            key_rollout, key_shuffle = rand.split(key)

            model = eqx.combine(update_state.param_state.params, network_static)

            # collect rollouts
            def rollout(init_rollout_state: RolloutState, key: rand.PRNGKey):
                @partial(
                    jax.lax.scan,
            ...
            rollout_state, trajectories, advantages = jax.vmap(rollout)(
            ...
            def epoch_step(param_state: ParamState, key: rand.PRNGKey):
                ...
                def gradient_step(
                    ...
                    def trajectory_loss(params: ActorCriticRNN, trajectory: Minibatch):
                        model = eqx.combine(params, network_static)
                        hidden, trajectories, advantages = trajectory

                        _, (logits, value) = model(
                            hidden,
                            ObsWithDone(trajectories.obs, trajectories.done),
                        )

                        # value loss
                        clipped_values = trajectories.value + jnp.clip(
                            value - trajectories.value,
                            -config.clip_epsilon,
                            config.clip_epsilon,
                        )
                        target_values = trajectories.value + advantages
                        value_loss = jnp.mean(
                            jnp.maximum(
                        ...
                        # policy loss
                        action_idx = (jnp.arange(advantages.size), trajectories.action)
                        ratios = jnp.exp(
                            logits[action_idx] - trajectories.logits[action_idx]
                        )
                    ...
                    def loss_fn(params: ActorCriticRNN):
                        return jnp.mean(
                            jax.vmap(trajectory_loss, in_axes=(None, 0))(
                                params, minibatch
                            )
                        )

                    loss, grad = jax.value_and_grad(loss_fn)(update_state.params)
