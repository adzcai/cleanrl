...
def metrics_wrapper(
    ...
    while getattr(env_inner, "_inner", None) is not None:
        if env_inner.name == "auto_reset":
            raise ValueError("Metrics wrapper should be applied before auto_reset wrapper.")
    ...
    def reset(params: TEnvParams, *, key: Key[Array, ""]) -> TimeStep[TObs, LogState[TEnvState]]:
