...
def new_housemaze():
    ...
    if False:
        ...
        map_init = [
            maze_utils.from_str(map_str, char_to_key=char_to_key, object_to_index=object_to_index) for map_str in levels
        ]
...
def housemaze_wrapper(
    ...
    def observation_space(params: maze.EnvParams) -> PyTree[specs.Array]:
        ...
        return HouseMazeObs(
            ...
            direction=specs.BoundedArray.discrete(len(maze.DIR_TO_VEC), name="direction"),
            ...
            prev_action=specs.BoundedArray.discrete(env.num_actions(params) + 2, name="prev_action"),
        )
    ...
    def step(
        ...
        *,
        ...
        step_type = jnp.select(
            [env_timestep.step_type == tp for tp in [maze.StepType.FIRST, maze.StepType.MID, maze.StepType.LAST]],
    ...
    return Environment(
        ...
        reset=reset,
        step=step,
        action_space=lambda params: specs.BoundedArray.discrete(env.num_actions(params), name="action"),
        observation_space=observation_space,
        goal_space=lambda params: specs.BoundedArray.discrete(len(params.objects), name="goal"),
    )
...
def visualize_housemaze(
    ...
    video: Float[Array, " height width channel"] = jax.vmap(renderer.create_image_from_grid, in_axes=(0, 0, 0, None))(
        timestep.state.grid,
        (r, c),
