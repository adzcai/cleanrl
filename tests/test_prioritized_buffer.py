...
def test_add_and_sample(buffer, dummy_experience):
    ...
    batch = {"obs": jnp.arange(buf.batch_size * buf.sample_len * 4).reshape(buf.batch_size, buf.sample_len, 4)}
...
def test_multiple_priority_updates(buffer, dummy_experience):
    ...
    for i in range(1, 11):
        ...
        root_val = state.priority_state.nodes[0]
        expected_sum = priorities.sum()
        assert jnp.isclose(root_val, expected_sum), f"Cycle {i}: Root {root_val}, sum {expected_sum}"
...
def test_new_experience_priority_is_max(buffer, dummy_experience):
    ...
    next_idx = jax.vmap(buf.pos_to_flat)(
        jnp.ones((buf.batch_size,), dtype=int)[jnp.newaxis, :] + jnp.arange(2)[:, jnp.newaxis]
    ).ravel()
    priorities = state.priority_state.nodes[buf.priority_tree.leaf_idx :]
    desired = jnp.zeros_like(priorities).at[idx_pos_zero].set(new_priorities).at[next_idx].set(jnp.max(new_priorities))
    npt.assert_array_equal(priorities, desired)
...
def test_buffer_overfill_priorities(buffer, dummy_experience):
    ...
    priorities = state.priority_state.nodes[buf.priority_tree.leaf_idx : buf.priority_tree.leaf_idx + buf.max_time]
