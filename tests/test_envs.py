...
def test_grid_reward(simple_env):
...
def test_grid_draw(simple_env):
    π = distrax.Categorical(logits=jnp.zeros((simple_env.S, simple_env.A)))
    simple_env.draw(π, "artifacts/test_grid_draw.png", "simple map")


def test_grid_value_iteration(env):
    Q = env.value_iteration()
    π = Q.argmax(axis=1)
    index = jnp.arange(env.S), π
    π = jnp.zeros((env.S, env.A)).at[index].set(1)
    π = distrax.Categorical(probs=π)
    env.draw(π, f"artifacts/test_grid_value_iteration_{env.S}.png", "simple map")


def test_grid_return(env):
